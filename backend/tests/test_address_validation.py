...
class AddressValidationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_poi_returns_true_when_adcode_and_coordinate_match(self) -> None:
        address = make_address_valid_data()
        self.amap_client.get_poi_detail.return_value = make_poi()

        resolved_location, status = await self.validation.address_validation(address)
    ...
    async def test_poi_returns_false_when_adcode_does_not_match(self) -> None:
        address = make_address_valid_data()
        ...
        self.assertEqual(resolved_location.adcode, "440304")

    async def test_poi_returns_true_when_distance_is_within_200_meters(self) -> None:
        address = make_address_valid_data(coordinate="113.934528,22.540503")
        self.amap_client.get_poi_detail.return_value = make_poi(
            coordinate="113.934528,22.542000"
        )

        _, status = await self.validation.address_validation(address)

        self.assertTrue(status)

    async def test_poi_returns_false_when_distance_exceeds_200_meters(self) -> None:
        address = make_address_valid_data(coordinate="113.934528,22.540503")
        self.amap_client.get_poi_detail.return_value = make_poi(
    ...
    async def test_position_returns_true_when_reverse_geocode_adcode_matches(
        self,
    ) -> None:
    ...
    async def test_position_returns_false_when_reverse_geocode_adcode_differs(
        self,
    ) -> None:
        address = make_address_valid_data(
    ...
    async def test_position_with_poi_returns_true_when_poi_is_within_500_meters(
        self,
    ) -> None:
        address = make_address_valid_data(
            source="position", amap_poi_id="B0NEARBY"
    ...
    async def test_position_with_poi_returns_false_when_poi_exceeds_500_meters(
        self,
    ) -> None:
        address = make_address_valid_data(
            source="position", amap_poi_id="B0NEARBY"
